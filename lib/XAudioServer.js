...
export function remainingBuffer() {
	...
	return (Math.floor((resampledSamplesLeft * resampler.ratioWeight) / numChannels) * numChannels) + currentBufferSize;
}
...
function initializeWebAudio() {
	...
	scriptProcessor.onaudioprocess = function (event) {		//Web Audio API callback...
		...
		if (currentBufferSize > 0) {
			...
			var resampleLength = resampler.resampler(bufferSamples);
			var resampledResult = resampler.outputBuffer;
	...
	resampler = new Resampler(sourceSampleRate, destSampleRate, numChannels, resamplerBufferSize, true);
