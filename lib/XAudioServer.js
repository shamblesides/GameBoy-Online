...
export function init(_minBufferSize, _maxBufferSize) {
	console.log(samplesPerCallback);
	console.log(_minBufferSize, _maxBufferSize);
	...
	console.log(minBufferSize, maxBufferSize);
	initializeWebAudio();
}
...
function initializeWebAudio() {
	...
	scriptProcessor.onaudioprocess = function (event) {		//Web Audio API callback...
		...
		if (currentBufferSize > 0) {
			...
			if (bufferSamples[0] !== -1) t_end();
			for (let i = 0; i < resampleLength; i++) {
				resampledBuffer[resampleBufferEnd++] = bufferSamples[i];
	...
	resamplerBufferSize = Math.max(maxBufferSize, samplesPerCallback * numChannels);
	console.log(resamplerBufferSize);
