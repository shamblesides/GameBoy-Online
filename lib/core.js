		...
		if (clockUpTo === 0) {
			...
			while (song.length > 0 && !(song[0] > 0)) {
				if (song[0] === loopStart) {
					songLooping = true;
					song.shift();
				} else if (typeof song[0] === 'function') {
					song[0]();
					if (songLooping) song.push(song.shift());
					else song.shift();
				} else if (song[0] === 0) {
					song.shift();
				} else {
					throw new Error("Unknown token in song: " + song[0])
				}
			}
			clockUpTo = Math.min(audioClocksUntilNextEventCounter, sequencerClocks);
			if (!(clockUpTo > 0)) throw new Error(`gameboy-audio: core.js: bad state (clockUpTo = ${clockUpTo})`)
